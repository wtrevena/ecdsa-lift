...
$5$--$11\sigma$ anomaly on the original sweep (and $\sim$$3$--$14\sigma$ on a clean
controlled set, with a standardized effect size $\approx44$---against a
$300$-draw empirical null, not a Gaussian tail---on a confirmatory curve
with $n\approx 10^4$) that a standard permutation control certified
...
not determined by the public data $(E,G,Q)$ over the natural lift (and,
we note separately from the theorem, for which no efficient algorithm
from $(E,G,Q)$ is known). The
single inequality $\gcd(n,p)=1$ is sharp: when it fails ($n=p$, anomalous
curves) it is the \emph{ambiguity} that is annihilated---$[p]c=O$ for
every kernel element $c$---so $[p]\tau(G)$ becomes a section-independent,
secret-linear invariant and the same machinery recovers $k$ with $100\%$
success---Smart's attack. The contribution is the methodology a
...
no known efficient algorithm computes from it---a state-of-knowledge
remark, kept separate from the theorem). We present it as the explicit
instance it is of \cite{silvermanlift,gadiyarpadma}, with proof and
verification.
...
within $\approx1$ standard deviation of that mean. Theorem~\ref{thm:main}
forecloses its being cryptanalytic in any case (a deterministic
post-processing of the public digit map of a public object cannot leak
the secret). The decisive evidence that
H2 is the mechanism is the $\Uk^2$ match, which is genuine.
A post-hoc confirmatory run on a large-prime-order curve ($p=10477$,
$n=10639$ prime; Phase 47, last row of Table~\ref{tab:p43}) extends the
pattern by more than an order of magnitude in $n$: the real elevation
reaches $+43$--$44$, the reflection-only synthetics track it at
$+44.5$--$44.8$, and real vs.\ synthetic stays within $\pm2.1$.
...
Let $f\colon\Z/L\to S^1$. For $f$ with i.i.d.\ $S^1$-uniform entries,
$\mathbb E\big[\|f\|_{\Uk^2}^4\big]=2/L+o(1/L)$. For $f$ subject to
\emph{only} the antisymmetry constraint $f(L{+}1{-}k)=\beta\,\overline{f(k)}$
($\beta\in S^1$ fixed, the free half i.i.d.\ $S^1$-uniform)---the
reflection-only synthetic of the positive control---%
$\mathbb E\big[\|f\|_{\Uk^2}^4\big]=3/L+o(1/L)$. The ratio is exactly
$3/2$, independent of $L$ and of the value alphabet. Consequently the
standardized synthetic-vs-random gap grows as
$z(\mathrm{syn/rand})=\Theta(\sqrt L)$.
\end{proposition}

\begin{proof}[Proof sketch]
$\|f\|_{\Uk^2}^4=\sum_\xi|\hat f(\xi)|^4$ with
$\hat f(\xi)=\tfrac1L\sum_k f(k)e^{-2\pi ik\xi/L}$. For i.i.d.\ $f$ the
$L-1$ nonzero modes are asymptotically independent circularly-symmetric
complex Gaussians with $\mathbb E|\hat f(\xi)|^2=1/L$, so
$\mathbb E|\hat f(\xi)|^4=2/L^2$ (two Wick pairings) and the sum is
$2/L$. The constraint forces $\hat f(L{-}\xi)=\beta\,\overline{\hat
f(\xi)}$, coupling each mode to its index-reversal partner; this added
second-order structure raises $\mathbb E|\hat f(\xi)|^4$ from $2/L^2$ to
$3/L^2$ (the coefficient $3$ verified by direct second-moment
measurement, Phase~48), and the sum to $3/L$. The
mean $\Uk^2$ ratio is then $(3/2)^{1/4}$; as the random-null standard
deviation scales as $\Theta(L^{-3/4})$, the standardized gap scales as
$\sqrt L$.
\end{proof}

Numerically the constant is $z(\mathrm{syn/rand})\approx0.43\sqrt n$
(verified $p$- and $L$-independent, $L\in[78,10638]$; Phase~48), and
$0.43\sqrt n$ reproduces the syn/rand column of Table~\ref{tab:p43} and
the Phase-47 value (predicted $44.8$ vs.\ observed $44.6$ at $n=10639$)
within finite-size noise. So $\sqrt n$ growth is the \emph{signature of
the antisymmetry artifact}, not of an accumulating signal---the exact
inference the original report inverted. The inflation is specific to
the \emph{index-reversing} coupling of \eqref{eq:antisym}: a constraint
sending mode $\xi$ to its reversal $-\xi$ raises the mass to $3/L$,
whereas one sending it elsewhere (e.g.\ a fixed shift) leaves it at
$2/L$---a discriminator an auditor can apply directly.
...
$\varphi(G)=15$---and Smart's attack recovers $k$ for $12/12$ random
secrets. The same code path that is inert for $\gcd(n,p)=1$ is a total
break the instant $n=p$.
...
Anomalous (Smart), $n=p$ & $y^2{=}x^3{+}4x{+}7/\mathbb F_{53}$ & zero map &
  $\neq O$, section-indep., $\propto k$ & broken, $12/12$\\
\bottomrule
\end{tabular}
